...
/**
 * Public (BFF) API
 * - Cookie-sessies (HttpOnly) + automatische SameSite-detectie
 * - CSRF beschermingsheader
 * - CORS (allowed_origins) met credentials
 * - PHPMailer via SMTP (verify/reset)
...
function compute_cookie_params(): array {
  ...
  $cross = false;
  if ($oHost && $sHost) {
    $cross = !hash_equals(strtolower($oHost), strtolower($sHost));
    // Als cookie_domain is ingesteld en beide hosts eindigen op dat domain, beschouw als same-site
  ...
  if ($same === 'None') {
    // Browsers eisen Secure bij SameSite=None
    $secure = true;
  }
  ...
  return [
    ...
    'domain'   => $cookieDomain ?: '',
    'secure'   => $secure,
    'httponly' => true,     // voor sessiecookie
    'samesite' => $same
  ];
}
...
function send_mail(string $toEmail, string $subject, string $html): void {
  try{
    ...
    // hier bewust geen bad(); e-mail failure mag login niet hard breken
  }
}

// ---------- Proxy helper naar Internal API ----------
function call_internal(string $path,string $method,?array $payload,?string $userId){
    $cfg=cfg();
    $base = rtrim((string)$cfg['backend_url'],'/');
    // Sta zowel admin-root (backend_admin.php) als internal_api.php toe; normaliseer naar internal_api.php
    if (str_ends_with($base,'/backend_admin.php')) {
        $base = substr($base, 0, -strlen('/backend_admin.php')).'/internal_api.php';
    }
    $url=$base.$path;
    $ch=curl_init($url);
...
if (($seg[0]??'')==='auth' && $method==='POST' && ($seg[1]??'')==='register') {
    ...
    send_mail($r['email'],'Bevestig je e-mailadres', '<p>Bevestig je e-mailadres voor Timelog:</p><p><a href="'.$verifyLink.'">E-mailadres bevestigen</a></p><p>Deze link verloopt binnenkort.</p>');
